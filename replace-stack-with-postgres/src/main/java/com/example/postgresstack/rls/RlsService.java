...
public class RlsService {
    ...
    public record AddResult(List<Note> notes, int inserted) {}
    ...
    public List<Note> notesFor(long uid) {
        jdbc.getJdbcTemplate().execute("SET LOCAL app.current_user = " + uid);
        return jdbc.query("""
    ...
    public AddResult addNote(long uid, String content, String ownerOverride) {
        jdbc.getJdbcTemplate().execute("SET LOCAL app.current_user = " + uid);
        long ownerId = ownerOverride == null || ownerOverride.isBlank()
            ? uid
        ...
        return new AddResult(notesForIntx(uid), inserted);
    }

    private List<Note> notesForIntx(long uid) {
        return jdbc.query("""
            SELECT n.id, n.content, n.created_at, u.name AS owner_name
            FROM private_notes n
            JOIN app_users u ON u.id = n.owner_id
